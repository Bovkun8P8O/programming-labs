namespace Lab5
{
    internal class Manager(int ID, string Name, int Salary, int Experience, int HolidaysPerYear) : IManager, IExecutor
    {
        private int Id = ID;
        private int bossID;
        ...
        public void Work()
        {
            Console.WriteLine($"Manager {ID}: {Name} is working");
        }
        public void Resign()
        {
            Console.WriteLine($"Manager {ID}: {Name} is resigning");
            ID = 0;
        }
        public void TakeVacation()
        {
            Console.WriteLine($"Manager {ID}: {Name} is taking a vacation");
        }
        ...
        {
            if (executor is Mechanic || executor is Engineer)
            {
                subordinates.Add(executor);
                Console.WriteLine($"Manager {ID}: {Name} added subordinate {executor.ID}: {executor.Name}");
                executor.BossID = ID;
                return;
            }
            Console.WriteLine($"Manager cannot add {executor.Position} as a subordinate");
        }
        public void RemoveSubordinate(IExecutor executor)
        {
            subordinates.Remove(executor);
            Console.WriteLine($"Manager {ID}: {Name} removed subordinate {executor.ID}: {executor.Name}");
        }
        public void Dismiss(IExecutor executor)
        {
            Console.WriteLine($"Manager {ID}: {Name} is dismissing {executor.ID}: {executor.Name}");
